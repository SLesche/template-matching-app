function [legend_text, title_text, subtitle_text] = return_plot_legend(app, approach, b, latency, fit_cor)     
    if approach == "maxcor"
        ...
        title_text = {"Individual ERP matched to grand average via minimizing the squared distance", strcat("ERP Number: ", num2str(app.erp_num))};
        legend_text = ["ERP num", "Matched Grand Average Waveform", strcat("Correlation Fitted ", app.cfg.component_name)];
    ...
    elseif approach == "minsq"
        proper_latency = round(latency, 2);
        title_text = {"Individual ERP matched to grand average via minimizing the squared distance", strcat("ERP Number: ", num2str(app.erp_num))};
        legend_text = ["ERP num", "Matched Grand Average Waveform", strcat("Minsq Fitted ", app.cfg.component_name)];
